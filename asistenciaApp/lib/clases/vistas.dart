...
Musuario usuario;
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Container(margin:EdgeInsets.all(5), child:Text(usuario.datos, style: TextStyle(fontWeight: FontWeight.bold))),
            Container(margin:EdgeInsets.all(5), child:Text(usuario.tipo, style: TextStyle(fontWeight: FontWeight.bold))),
            Expanded(
                child: Divider(color: Colors.black,
                    thickness: 2,)
            ),
            elementoMenu("INICIO", "", "principal",this.usuario),
            elementoMenu("ALUMNOS", "", "alumnos",this.usuario),
            elementoMenu("MATRICULAS \n- NOTAS \n- ASISTENCIAS \n- INCIDENCIAS",
                "", "matriculas",this.usuario),
  ...
  void salto(vista,Musuario usuario) {
    switch (vista) {
      ...
      case "alumnos":
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => alumnos(usuario: usuario)));
        break;
      case "matriculas":
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => matriculas(usuario: usuario)));
        break;
      case "cerrar":
        sesion ses = sesion();
        ses.cerrarSesion();
